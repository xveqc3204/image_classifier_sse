...
from model import create_model, create_data_loaders, ModelEvaluator, print_model_summary
...
class SSETrainer:
    """Main trainer class for the SSE image classifier"""
    ...
    def plot_training_history(self):
        ...
        # Learning rate plot
        ax3.plot(epochs, self.history['learning_rate'], 'g-')
        ax3.set_title('Learning Rate Schedule')
        ax3.set_xlabel('Epoch')
        ax3.set_ylabel('Learning Rate')
        ax3.set_yscale('log')
        ...
        ax4.plot(epochs, self.history['train_accuracy'], 'b-', label='Training', alpha=0.7)
        ax4.plot(epochs, self.history['val_accuracy'], 'r-', label='Validation', alpha=0.7)
        ax4.fill_between(epochs, self.history['train_accuracy'], alpha=0.3, color='blue')
        ax4.fill_between(epochs, self.history['val_accuracy'], alpha=0.3, color='red')
    ...
    def evaluate_test_set(self):
        ...
        evaluator = ModelEvaluator(self.model, self.device)
        metrics, predictions, labels, probabilities = evaluator.evaluate(self.test_loader)
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                   xticklabels=['Chemical', 'Biological'],
                   yticklabels=['Chemical', 'Biological'])
...
def main():
    """Main function to run training"""
    print("Image Classifier SSE - Training Script")
    print("=" * 50)
...
class LabSafetyTrainer:
    """Main trainer class for the lab safety classifier"""
...
def main():
    """Main function to run training"""
    print("Lab Safety Image Classifier - Training Script")
    print("=" * 50)
