...
if __name__ == "__main__":
    main()("No image files found in the directory")
            return
        ...
        if not any('error' in r for r in results):
            ...
            chemical_count = predictions.count('Chemical')
            biological_count = predictions.count('Biological')
            
            print"""
Inference script for Lab Safety Image Classifier
Use trained model to classify new images
"""
...
def create_gradio_interface(predictor):
    ...
    interface = gr.Interface(
        fn=predict_and_visualize,
        inputs=gr.Image(type="pil", label="Upload Lab Safety Image"),
        outputs=[
            gr.Markdown(label="Prediction Results"),
            gr.Label(label="Classification")
        ],
        title="Lab Safety Image Classifier",
        description="""
        Upload an image containing lab equipment or safety items to classify it as either:
        - **Chemical**: Related to chemical handling, storage, or safety
        - **Biological**: Related to biological research or safety
...
def main():
    """Main function for inference"""
    parser = argparse.ArgumentParser(description='Lab Safety Image Classifier - Inference')
    parser.add_argument('--model', type=str, 
                       default='models/saved_models/best_model.pth',
                       help='Path to trained model')
    parser.add_argument('--image', type=str, 
    ...
    # Create predictor
    print("Loading model...")
    predictor = LabSafetyPredictor(args.model, args.config)
    print("Model loaded successfully!")
